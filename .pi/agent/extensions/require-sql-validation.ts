...
const WRITE_RE = /\b(?:insert\s+into|update\s+\w+|delete\s+from|merge\s+into|alter\s+(?:table|schema|database)|create\s+(?:table|schema|database|index)|drop\s+(?:table|schema|database|index)|truncate\s+(?:table\s+)?\w+|alembic\s+(?:upgrade|downgrade))\b/i;
const SELECT_PROBE_RE = /\bselect\b[\s\S]*\bwhere\b/i;
const SQL_TEXT_RE = /\b(?:select\b[\s\S]*\bfrom\b|insert\s+into|update\s+\w+|delete\s+from|merge\s+into|alter\s+(?:table|schema)|create\s+(?:table|schema)|drop\s+(?:table|schema)|truncate\s+)/i;
